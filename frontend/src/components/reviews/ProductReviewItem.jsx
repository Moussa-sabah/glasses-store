...
import EditReview from '../../modals/Review/EditReviewOfProduct'
import { deleteReviewOfProduct, getOneReviewOfProduct } from '../../redux/apiCalls/ProductApiCall'
...
export default function ReviewItem({ review, reviewUser }) {
  ...
  const OpenToEditReviewModal = (id) => {
    dispatch(getOneReviewOfProduct(review?._id))
    setTimeout(() => {
  ...
  const deleteReview = () => {
    swal({
      ...
      .then((isOk) => {
        if (isOk) {
          dispatch(deleteReviewOfProduct(review?._id))
        }
